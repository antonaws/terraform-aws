...
locals {
  ...
  # e.g., var.vpc_cidr = "10.1.0.0/21" => output: ["10.1.0.0/24", "10.1.1.0/24"] => 254 usable IPs per subnet/AZ
  ...
  # e.g., var.vpc_cidr = "10.1.0.0/21" => output: ["10.1.2.0/26", "10.1.2.64/26"] => 62 usable IPs per subnet/AZ
  ...
  # e.g., var.secondary_cidr_blocks = "100.64.0.0/16" => output: ["100.64.0.0/17", "100.64.128.0/17"] => 32766 usable IPs per subnet/AZ
...
module "vpc" {
  ...
  # Public Subnets for NAT and IGW for PoC/Dev/Test environments
  ...
  single_nat_gateway   = true  # Use single NAT for cost savings in dev, set to false for production
